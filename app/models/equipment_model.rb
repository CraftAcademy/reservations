...
class EquipmentModel < ActiveRecord::Base
  ...
  def num_busy(start_date, due_date, source)
    ...
    start_date.upto(due_date) do |d|
      busy = Reservation.number_for(source, date: d, equipment_model_id: id,
                                    overdue: false)
      max = busy if busy > max
    end
    max + overdue_count
  end
